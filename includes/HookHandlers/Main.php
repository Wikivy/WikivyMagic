...
namespace Wikivy\WikivyMagic\HookHandlers;
	...
	UserGetRightsRemoveHook
{
	...
	public static function factory(
		...
		return new self(
			...
			new ServiceOptions(
				[
					'WikivyMagicAccessIdsMap',
					'WikivyReportsBlockAlertKeywords',
					'WikivyReportsWriteKey',
	...
	public function onSkinAddFooterLinks( Skin $skin, string $key, array &$footerItems ) {
		if ( $key === 'places' ) {
			...
			$footerItems['donate'] = $this->addFooterLink( $skin, 'wikivy-donate', 'wikivy-donatepage' );
		}
	}
	...
	public function onUserGetRightsRemove( $user, &$rights ) {
		...
		foreach ( $this->options->get( 'WikivyMagicAccessIdsMap' ) as $wiki => $ids ) {
	...
	public function onSiteNoticeAfter( &$siteNotice, $skin ) {
		...
		if ( $cwConfig->get( 'Closed' ) ) {
			if ( $cwConfig->get( 'Private' ) ) {
				$this->appendNotice(
					$siteNotice,
					$skin->msg( 'wikivy-sitenotice-closed-private' )->parse(),
			...
			if ( $cwConfig->get( 'Locked' ) ) {
				$this->appendNotice(
					$siteNotice,
					$skin->msg( 'wikivy-sitenotice-closed-locked' )->parse(),
			...
			$this->appendNotice(
				$siteNotice,
				$skin->msg( 'wikivy-sitenotice-closed' )->parse(),
				'https://static.wikitide.net/metawiki/0/02/Wiki_lock.png'
			);
			return;
		}
		...
		if ( $inactive && $inactive !== 'exempt' ) {
			$msgKey = $cwConfig->get( 'Private' )
				? 'wikivy-sitenotice-inactive-private'
				: 'wikivy-sitenotice-inactive';
	...
	public function onRecentChange_save( $recentChange ) {
		...
		$data = [
			'writekey' => $this->options->get( 'WikivyReportsWriteKey' ),
		...
		$this->httpRequestFactory->post( 'https://reports.wikivy.com/api/ial', [ 'postData' => $data ], __METHOD__ );
	}
	...
	public function onBlockIpComplete( $block, $user, $priorBlock ) {
		...
		$blockAlertKeywords = $this->options->get( 'WikivyReportsBlockAlertKeywords' );
		foreach ( $blockAlertKeywords as $keyword ) {
			...
			if ( str_contains( mb_strtolower( $block->getReasonComment()->text ), mb_strtolower( $keyword ) ) ) {
				...
				$this->httpRequestFactory->post( 'https://reports.wikivy.com/api/report', [ 'postData' => $data ], __METHOD__ );
				return;
			}
		}
	}
