...
class GenerateWikivySitemap extends Maintenance
{
	...
	public function __construct()
	{
		...
		$this->addDescription('Generates sitemap for all Wikivy wikis (apart from private ones).');
	}
	...
	public function execute()
	{
		...
		$statusFormatter = $this->getServiceContainer()
			->getFormatterFactory()
		...
		if ($isPrivate) {
			...
			$sitemaps = $backend->getTopFileList([
				'dir' => $localRepo->getZonePath('public').'/sitemaps', 'adviseStat' => false,
			]);
			...
			foreach (glob($filePath.'/sitemap-*'.$dbname.'*') as $sitemap) {
				$backend->quickStore([
					'src' => $sitemap, 'dst' => $localRepo->getZonePath('public').'/sitemaps/'.basename($sitemap),
				]);
...
return GenerateWikivySitemap::class;
